...
<head>
    ...
    <link rel="stylesheet" href="css/dashcheck.css">
...
<body class="loading" data-layout="topnav" data-layout-config='{"layoutBoxed":false,"darkMode":false,"showRightSidebarOnStart": true}'>
    ...
    <br>
    <h3 class="header-title mb-4" style="color: #F7931E;">
...
<div class="container-fluid text-center">
    <div class="border p-3 mt-4 mt-lg-0 rounded" style="background-color: #242e3b; width: 50%; margin: 0 auto;">
                ...
                <tbody>
                                    ...
                                    if (mysqli_num_rows($sql) > 0) {
                                        ...
                                        while ($row = mysqli_fetch_assoc($sql)) {
                                            ...
                                            // Use $row['order_status'] instead of $order['order_status']
                                            if (!isset($orders[$order_id]) || $row['order_date'] > $orders[$order_id]['order_date']) {
                                                $orders[$order_id] = array(
                                                    'order_id' => $order_id,
                                                    'status' => $row['order_status'],
                                        ...
                                        foreach ($orders as $order) {
                                    ?>
                                            <div style="max-height: 230px; border-bottom: 1px solid #ddd;" data-simplebar="">
                                                <!-- Use $row['order_status'] instead of $order['order_status'] -->
                                                <a href="order_customer.php" class="dropdown-item notify-item" style="color: white;">
                                                    <!-- Removed the icon -->
                                                    <p class="notify-details mb-2" style="font-size: 18px;">Your Order # <?php echo $order['order_id']; ?> has the
                                                        status of <span class="badge <?php echo isset($statusBadgeClasses[$order['status']]) ? $statusBadgeClasses[$order['status']] : 'badge-info-lighten'; ?>">
                                                            <?php echo $order['status']; ?>
                                                        </span></p>
                                                    <small class="text-muted" style="font-size: 14px;"><?php echo $order['order_date']; ?></small>
                                                </a>
                                            </div>
                                    <?php
                                        }
                                        ...
                                        echo "<a href='#' class='dropdown-item notify-item' style='color: #495057;'>";
                                        echo '    <p class="notify-details" style="font-size: 18px;">No orders</p>';
                                        echo '</a>';
                                    }
                                    ?>
                                </tbody>
